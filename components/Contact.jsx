...
export default function Contact() {
  ...
  const contacts = [
    { href: personalInfo.links.email,   label: "Email",   sub: personalInfo.links.email,   Icon: Mail,    accent: "text-red-400",    bg: "bg-red-950/40",    border: "border-red-900/60" },
    { href: personalInfo.links.github,  label: "GitHub",  sub: personalInfo.links.github,  Icon: Github,  accent: "text-rose-400",   bg: "bg-rose-950/30",   border: "border-rose-900/40" },
    { href: personalInfo.links.linkedin,label: "LinkedIn",sub: personalInfo.links.linkedin,Icon: Linkedin, accent: "text-orange-400", bg: "bg-orange-950/30", border: "border-orange-900/40" },
  ];
              ...
              <div className="space-y-3">
                {contacts.map(({ href, label, sub, Icon, accent, bg, border }) => (
                  <a key={label} href={href} target="_blank" rel="noopener noreferrer"
                    className={`flex items-center gap-4 p-4 rounded-xl bg-[#0D0D0D] border border-[#3F1111] hover:border-[#6B1A1A] text-[#8A8A8A] hover:text-[#F5F5F5] transition-all group`}
                  >
                    <div className={`p-3 rounded-lg ${bg} ${accent} ${border} border group-hover:scale-110 transition-transform`}>
                      <Icon className="w-5 h-5" />
                    </div>
                    <div>
                      <span className="text-xs text-[#8A8A8A] block font-mono">{label}</span>
                      <span className={`text-sm font-semibold group-hover:${accent} transition-colors`}>{sub}</span>
                    </div>
                  </a>
                ))}
              </div>
            ...
            <form onSubmit={handleSubmit} className="glass-card p-8 sm:p-10 rounded-2xl space-y-6">
              ...
              {[
                ...
                <div key={id} className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-[#050505] border border-[#3F1111] text-[#F5F5F5] placeholder-[#8A8A8A]/50 focus:outline-none focus:border-red-700 focus:ring-1 focus:ring-red-800 text-sm transition-colors"
                  />
                </div>
              ))}
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 rounded-xl bg-[#050505] border border-[#3F1111] text-[#F5F5F5] placeholder-[#8A8A8A]/50 focus:outline-none focus:border-red-700 focus:ring-1 focus:ring-red-800 text-sm transition-colors resize-none"
                />
              </div>
